...
function AlertBanner({
  ...
  return (
    <div
      className={clsx(
        "dashboard-card flex items-start gap-3 rounded-lg border p-4",
        tone === "amber"
    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        {properties.map((property) => (
          <article
            key={property.name}
            className="dashboard-card overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.06)] transition hover:-translate-y-0.5 hover:shadow-[0_22px_60px_rgba(15,23,42,0.10)]"
          >
            <img
              src={property.image}
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3 px-2">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Switch between tenant and owner experiences
                </p>
              </div>
            </div>
            <DemoContent />
