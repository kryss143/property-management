...
export async function getDashboard(token?: string): Promise<DashboardPayload> {
  ...
  return request<DashboardPayload>("/dashboard", token).catch(() => demoDashboard);
}

export async function listResource<T>(resource: string, token?: string): Promise<ListResponse<T>> {
  if (!token) {
...
export async function createResource<T>(resource: string, token: string | undefined, payload: Record<string, unknown>) {
  if (!token) {
    return { ...payload, id: crypto.randomUUID(), created_at: new Date().toISOString() } as T;
  }
...
export async function deleteResource(resource: string, id: string, token?: string) {
  if (!token) return;
